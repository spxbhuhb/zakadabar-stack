 ...
 */
package zakadabar.cookbook.jvm.client
...
suspend fun crud() {
    ...
    val newBuiltin = ExampleBo.default {
    ...
    val builtinRead = ExampleBo.read(newBuiltin.id)
    builtinRead.secretValue = Secret("bbb") // without secret, validation fails
...
suspend fun dumpBuiltins(message: String) {
    println("\n    ---- $message ----\n")
    ExampleBo.all().forEach { println("        $it") }
}

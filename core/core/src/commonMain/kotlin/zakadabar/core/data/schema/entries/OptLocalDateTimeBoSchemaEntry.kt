...
import zakadabar.core.data.schema.descriptor.LocalDateTimeBoConstraint
import zakadabar.core.data.schema.descriptor.LocalDateTimeBoProperty
...
class OptLocalDateTimeBoSchemaEntry(val kProperty: KMutableProperty0<LocalDateTime?>) : BoSchemaEntry<LocalDateTime?> {
    ...
    @PublicApi
    infix fun max(limit: LocalDateTime): OptLocalDateTimeBoSchemaEntry {
        rules += Max(limit)
        return this
    }

    @PublicApi
    infix fun min(limit: LocalDateTime): OptLocalDateTimeBoSchemaEntry {
        rules += Min(limit)
        return this
    }

    @PublicApi
    infix fun before(limit: LocalDateTime): OptLocalDateTimeBoSchemaEntry {
        rules += Before(limit)
        return this
    }

    @PublicApi
    infix fun after(limit: LocalDateTime): OptLocalDateTimeBoSchemaEntry {
        rules += After(limit)
        return this
    }

    @PublicApi
    infix fun notEquals(invalidValue: LocalDateTime?): OptLocalDateTimeBoSchemaEntry {
    ...
    @PublicApi
    infix fun default(value: LocalDateTime?): OptLocalDateTimeBoSchemaEntry {
        defaultValue = value
        return this
    }

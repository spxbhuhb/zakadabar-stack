...
class ZkLiteDatabaseMetaData(var con: ZkLiteConnection) : DatabaseMetaData {
    ...
    override fun getCrossReference(pc: String, ps: String, pt: String?, fc: String, fs: String, ft: String?): ResultSet {
        ...
        val query = """
            select 
                ${quote(pc)} as PKTABLE_CAT,
                ${quote(ps)} as PKTABLE_SCHEM,
                ${quote(pt)} as PKTABLE_NAME,
                '' as PKCOLUMN_NAME,
                ${quote(fc)} as FKTABLE_CAT,
                ${quote(fs)} as FKTABLE_SCHEM,
                ${quote(ft)} as FKTABLE_NAME,
                '' as FKCOLUMN_NAME, 
                -1 as KEY_SEQ, 
                3 as UPDATE_RULE, 
                3 as DELETE_RULE, 
    ...
    override fun getExportedKeys(catalog: String?, schema: String?, table: String?): ResultSet {
            ...
            for (tbl in tableList) {
                ...
                try {
                    ...
                    while (fk.next()) {
                        ...
                        exportedKeysQuery += """
                            ${if (count > 0) " union all select " else "select "}
                                $keySeq as ks,
                                lower('${escape(tbl)}') as fkt, 
                                lower('${escape(fk.getString(4))}') as fcn,
                                '${escape(PKColName)}' as pcn, 
                                ${RULE_MAP[fk.getString(6)]} as ur,
                                ${RULE_MAP[fk.getString(7)]} as dr, 
                        """.trimIndent()
                        ...
                        if (rs.next()) {
                            ...
                            if (matcher.find()) {
                                exportedKeysQuery += "'${escape(matcher.group(1) !!.lowercase(Locale.getDefault()))}' as fkn"
                            } else {
                                exportedKeysQuery += "'' as fkn"
                            }
                        }
                        rs.close()
                        count ++
        ...
        var sql = """
                select
                    $qCatalog as PKTABLE_CAT,
                    $qSchema as PKTABLE_SCHEM,
                    ${quote(table)} as PKTABLE_NAME, 
                    ${if (hasImportedKey) "pcn" else "''"} as PKCOLUMN_NAME,
                    $qCatalog as FKTABLE_CAT,
                    $qSchema as FKTABLE_SCHEM,
                    ${if (hasImportedKey) "fkt" else "''"} as FKTABLE_NAME,
                    ${if (hasImportedKey) "fcn" else "''"} as FKCOLUMN_NAME,
                    ${if (hasImportedKey) "ks" else "-1"} as KEY_SEQ,
                    ${if (hasImportedKey) "ur" else "3"} as UPDATE_RULE,
                    ${if (hasImportedKey) "dr" else "3"} as DELETE_RULE,
                    ${if (hasImportedKey) "fkn" else "''"} as FK_NAME,
                    ${if (pkFinder.name != null) pkFinder.name else "''"} as PK_NAME,
                    ${DatabaseMetaData.importedKeyInitiallyDeferred} as DEFERRABILITY         
            """.trimIndent()// FIXME: Check for pragma foreign_keys = true ?

        sql += if (hasImportedKey) {
            " from (${exportedKeysQuery} order by fkt"
        } else {
            "limit 0"
        }
    ...
    override fun getImportedKeys(catalog: String, schema: String, table: String): ResultSet {
        ...
        val stat = con.createStatement()
        var sql = """
            select ${quote(catalog)} as PKTABLE_CAT,
            ${quote(schema)} as PKTABLE_SCHEM,
            ptn as PKTABLE_NAME,
            pcn as PKCOLUMN_NAME,
            ${quote(catalog)} as FKTABLE_CAT,
            ${quote(schema)} as FKTABLE_SCHEM,
            ${quote(table)} as FKTABLE_NAME,
            ...
            '' as FK_NAME, 
            '' as PK_NAME,
            ${DatabaseMetaData.importedKeyInitiallyDeferred} as DEFERRABILITY from (
        """.trimIndent()
        ...
        rs = try {
            ...
            sql += "select -1 as ks, '' as ptn, '' as fcn, '' as pcn, "
            sql += "${DatabaseMetaData.importedKeyNoAction} as ur, "
            sql += "${DatabaseMetaData.importedKeyNoAction} as dr) limit 0;"
            return stat.executeQuery(sql)

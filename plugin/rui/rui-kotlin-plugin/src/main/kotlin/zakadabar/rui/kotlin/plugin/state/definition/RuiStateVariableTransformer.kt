 ...
 * Transforms:
 *
 * - original function parameters into state variables
 * - top-level local variables into state variables
...
class RuiStateVariableTransformer(
    ...
    fun buildStateDefinition() {
        ...
        // transform value parameters into state variables

        ruiClass.original.valueParameters.forEach { old ->
            ruiClass.constructor.addValueParameter {
                name = old.name
                type = old.type
                varargElementType = old.varargElementType
            }.also { new ->
                ruiClass.addStateVariable(new).also {
                    blockStack[0].variables += it.originalName
                }
            }
        }

        // transform the top level variable declarations into class properties
        // transform all get and set into property getter/setter calls

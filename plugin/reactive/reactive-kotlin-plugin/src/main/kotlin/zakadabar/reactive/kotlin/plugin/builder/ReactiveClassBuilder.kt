 ...
 */
package zakadabar.reactive.kotlin.plugin.builder
...
import zakadabar.reactive.kotlin.plugin.data.ReactiveFunction
import zakadabar.reactive.kotlin.plugin.data.ReactiveVariable

class ReactiveClassBuilder(
    private val context: IrPluginContext,
    configuration: ReactivePluginContext
) {
    ...
    val reactiveComponentClass = context.referenceClass(FqName.fromSegments(listOf("zakadabar", "reactive", "core", "ReactiveComponent"))) !!
    val reactiveComponentType = reactiveComponentClass.typeWith()

    fun buildReactiveClass(file: IrFile, rFunction: ReactiveFunction): IrClass {
        return IrFactoryImpl.buildClass {
            ...
            parent = file
            superTypes = listOf(reactiveComponentType)
        ...
        addConstructor {
            ...
            body = irFactory.createBlockBody(SYNTHETIC_OFFSET, SYNTHETIC_OFFSET).apply {
                ...
                statements += IrDelegatingConstructorCallImpl.fromSymbolOwner(
                    ...
                    reactiveComponentClass.constructors.first()
                )
    ...
    internal fun IrClass.addInitializer(rFunction: ReactiveFunction) {
    ...
    private fun IrBlockBody.addVariableInitializer(irClass: IrClass, rVariable: ReactiveVariable) {
    ...
    internal fun IrClass.addVariables(rFunction: ReactiveFunction) {
        rFunction.variables.forEach {
            addVariable(this, it.value)
        }
    }

    internal fun addVariable(irClass: IrClass, rVariable: ReactiveVariable) {
        val irVariable = rVariable.irVariable
...
//    internal fun IrBlockBody.addPropertyInitialization(rFunction : ReactiveIrFunction) {
//
//    }
}

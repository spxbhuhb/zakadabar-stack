...
open class SQLDroidConnection(url: String, info: Properties?) : Connection {
    ...
    private fun ensureDbFileCreation(dbQname: String) {
        ...
        if (dbFile.isDirectory) {
        ...
        } else if (dbFile.parentFile.exists() && ! dbFile.parentFile.isDirectory) {
            ...
            if (! dbFile.parentFile.isDirectory) {
    ...
    override fun getMetaData(): DatabaseMetaData {
        return SQLDroidDatabaseMetaData(this)
    }
    ...
    override fun prepareStatement(sql: String, autoGeneratedKeys: Int): PreparedStatement {
        return SQLDroidPreparedStatement(sql, this, autoGeneratedKeys)
    }
    ...
    override fun setHoldability(holdability: Int) {
        if (holdability != ResultSet.CLOSE_CURSORS_AT_COMMIT) throw java.sql.SQLException("SQLDroid only supports CLOSE_CURSORS_AT_COMMIT")
    }
    ...
    override fun setTransactionIsolation(level: Int) {
        ...
        if (level != Connection.TRANSACTION_SERIALIZABLE) {
            throw java.sql.SQLException("SQLDroid supports only TRANSACTION_SERIALIZABLE.")
        }
    ...
    @Throws(java.sql.SQLException::class)
    override fun createClob(): SQLDroidClob {
...
//    }
    ...
    companion object {
         ...
         */
        private val dbMap: MutableMap<String?, SQLiteDatabase> = HashMap()
         ...
         */
        private val clientMap: MutableMap<SQLDroidConnection, SQLiteDatabase> = HashMap()
        ...
        fun chainException(sqlException: SQLException): java.sql.SQLException {
            if (sqlThrowable < 0 || sqlThrowable >= 9) {
                try {
                    ...
                    val c: Constructor<*> = java.sql.SQLException::class.java.getDeclaredConstructor(Throwable::class.java)
                    return c.newInstance(sqlException) as java.sql.SQLException
                } catch (e: Exception) {
                    sqlThrowable = 1
                }
            }
            ...
            return try {
                ...
                val c = SQLDroidConnection::class.java.classLoader.loadClass("org.sqldroid.SQLDroidSQLException").getDeclaredConstructor(
                ...
                c.newInstance(sqlException) as java.sql.SQLException
            } catch (e: Exception) {
    ...
    init {
        Log.v("SQLDroidConnection: " + Thread.currentThread().id + " \"" + Thread.currentThread().name + "\" " + this)
        ...
        if (info != null) {
            if (info.getProperty(SQLDroidDriver.DATABASE_FLAGS) != null) {
                try {
                    flags = info.getProperty(SQLDroidDriver.DATABASE_FLAGS).toInt()
                } catch (nfe: NumberFormatException) {
                ...
                try {
                    val extraFlags = info.getProperty(SQLDroidDriver.ADDITONAL_DATABASE_FLAGS).toInt()
